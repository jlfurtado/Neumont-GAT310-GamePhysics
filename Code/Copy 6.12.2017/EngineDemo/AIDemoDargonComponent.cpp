...
bool AIDemoDargonComponent::Update(float dt)
{
	...
	if (Engine::Keyboard::KeyIsUp(VK_SHIFT))
	{
		// change brain state if necessary
		for (int i = 0; i < 10; ++i)
		{
			if (Engine::Keyboard::KeyWasPressed('0' + i))
			{
				m_index = i;
		...
		if (Engine::Keyboard::KeyWasPressed('Y'))
		{
			m_index = 10;
			Engine::FSMPair pair = s_AIFuncs[m_index];
			m_brain.Push(pair.m_enter, pair.m_update, pair.m_exit, this);
		}
		else if (Engine::Keyboard::KeyWasPressed('T'))
		{
			m_index = Engine::MathUtility::Rand(0, NUM_FUNCS);
			Engine::FSMPair pair = s_AIFuncs[m_index];
			m_brain.Push(pair.m_enter, pair.m_update, pair.m_exit, this);
		}
		else if (Engine::Keyboard::KeyWasPressed('V'))
		{
			m_brain.Pop();
		}
	}
