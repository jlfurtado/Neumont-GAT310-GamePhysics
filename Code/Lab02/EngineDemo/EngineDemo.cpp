...
void EngineDemo::Update(float dt)
{
	...
	if (debug)
	{
		...
		if (m_sphereIndex >= 0)
		{
			...
			Engine::GameLogger::Log(Engine::MessageType::ConsoleOnly, "Momentum of sphere [%d]: (%.2f, %.2f, %.2f)\n", m_sphereIndex, m.GetX(), m.GetY(), m.GetZ());
		}
...
bool EngineDemo::ProcessInput(float /*dt*/)
{
	...
	if (m_sphereIndex >= 0)
	{
		if (Engine::Keyboard::KeyIsDown(VK_UP)) { m_objPhysics[m_sphereIndex].AddForce(Engine::Vec3(0.0f, 1.0f, 0.0f) * 1.0f); }
		if (Engine::Keyboard::KeyIsDown(VK_DOWN)) { m_objPhysics[m_sphereIndex].AddForce(Engine::Vec3(0.0f, -1.0f, 0.0f) * 1.0f); }
		if (Engine::Keyboard::KeyIsDown(VK_LEFT)) { m_objPhysics[m_sphereIndex].AddForce(Engine::Vec3(1.0f, 0.0f, 0.0f) * 1.0f); }
		if (Engine::Keyboard::KeyIsDown(VK_RIGHT)) { m_objPhysics[m_sphereIndex].AddForce(Engine::Vec3(-1.0f, 0.0f, 0.0f) * 1.0f); }
...
void EngineDemo::AlignObj(int index)
{
	...
	Engine::Vec3 pos = ORIGIN + Engine::MathUtility::GetCubification(index, OBJ_PER_DIR, OBJ_PER_DIR, 1, 50.0f);
