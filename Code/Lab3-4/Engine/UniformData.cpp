...
namespace Engine
{
	...
	void UniformData::PassFloatVec3(UniformData * pData)
	{
		...
		// log
		//if (pData->m_logForThis) { GameLogger::Log(MessageType::ConsoleOnly, "Passed (%.3f, %.3f, %.3f) to uniform [%d]\n", *reinterpret_cast<float*>((pData->m_pUniformData)), *(reinterpret_cast<float*>(pData->m_pUniformData) + 1), *(reinterpret_cast<float*>(pData->m_pUniformData) + 2), pData->m_uniformDataLoc); }
