...
namespace Engine
{
	...
	// TODO: UPGRADE TO FULL OUTPUT
	bool CollisionTester::RaySphereIntersect(const Vec3 & rayPosition, const Vec3 rayDirection, const Vec3 & sphereLoc, float radius)
	{
		// vector going to the sphere from the ray position
		Vec3 toSphere = sphereLoc - rayPosition;

		// sphere behind ray
		if (toSphere.Dot(rayDirection) < 0.0f) { return false; }

		// re-re-rejection!
		Vec3 rej = toSphere - toSphere.ProjectOnto(rayDirection);
		return rej.LengthSquared() < radius*radius;
	}
