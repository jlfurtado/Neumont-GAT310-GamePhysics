...
const Engine::Vec3 X_ARROW_OFFSET = Engine::Vec3::PLUS_X * (ARROW_SCALE + MOVE_MORE);
const Engine::Vec3 Y_ARROW_OFFSET = Engine::Vec3::PLUS_Y * (ARROW_SCALE + MOVE_MORE);
const Engine::Vec3 Z_ARROW_OFFSET = Engine::Vec3::PLUS_Z * (ARROW_SCALE + MOVE_MORE);
...
void WorldEditor::PlaceObject(WorldEditor *pEditor)
{
	if (Engine::MouseManager::IsLeftMouseClicked() && (pEditor->m_rco.m_didIntersect || pEditor->m_objs.GetCount() == 0))
	{
		...
		if (pEditor->m_objs.GetCount() != 0) { pNewObj->SetRotMat(Engine::Mat4::RotationToFace(Engine::Vec3::PLUS_Y, pEditor->m_rco.m_triangleNormal)); }
...
{
	Engine::Keyboard::Update(dt);
	if (!ProcessInput(dt)) { return; }
	ShowFrameRate(dt);
	
	if (m_walkEnabled)
	{
		Engine::RayCastingOutput groundRCO = Engine::CollisionTester::FindWall(m_camera.GetPosition() + (DOWN_CHECK - DOWN_OFFSET) * Engine::Vec3::PLUS_Y, -Engine::Vec3::PLUS_Y, CHECK_DIST, EDITOR_LIST_OBJS);

		if (groundRCO.m_didIntersect)
		{
			m_camera.SetPosition(m_camera.GetPosition() +  (Engine::Vec3::PLUS_Y * (DOWN_CHECK - groundRCO.m_distance)));
		}
...
bool WorldEditor::ProcessInput(float dt)
{
	...
	if (Engine::Keyboard::KeyIsDown('W')) { movementVector = movementVector + m_camera.GetViewDir().Normalize() * m_camera.GetSpeed() * dt; }
	if (Engine::Keyboard::KeyIsDown('S')) { movementVector = movementVector - m_camera.GetViewDir().Normalize() * m_camera.GetSpeed() * dt; }
	if (Engine::Keyboard::KeyIsDown('A')) { movementVector = movementVector - m_camera.GetViewDir().Cross(m_camera.GetUp()).Normalize() * m_camera.GetSpeed() * dt; }
	if (Engine::Keyboard::KeyIsDown('D')) { movementVector = movementVector + m_camera.GetViewDir().Cross(m_camera.GetUp()).Normalize() * m_camera.GetSpeed() * dt; }
	...
	if (Engine::Keyboard::KeyWasPressed('K') && Engine::Keyboard::KeyIsDown(VK_SHIFT))
	{
	...
	if (Engine::Keyboard::KeyWasPressed('1')) { SwapToPlace(); }
	if (Engine::Keyboard::KeyWasPressed('2')) { SwapToRemove(); }
	if (Engine::Keyboard::KeyWasPressed('3')) { SwapToTranslate(); }
	if (Engine::Keyboard::KeyWasPressed('4')) { SwapToRotate(); }
	if (Engine::Keyboard::KeyWasPressed('5')) { SwapToScale(); }
	if (Engine::Keyboard::KeyWasPressed('7')) { SwapMakeForward(); }
	if (Engine::Keyboard::KeyWasPressed('8')) { SwapMakeBackward(); }
	if (Engine::Keyboard::KeyWasPressed('9') && Engine::Keyboard::KeyIsDown(VK_SHIFT))
	{
	...
	if (Engine::Keyboard::KeyWasPressed('0') && Engine::Keyboard::KeyIsDown(VK_SHIFT))
	{
	...
	if (Engine::Keyboard::KeyWasPressed('M')) { m_adjustmentSpeedMultiplier *= 1.1f; }
	if (Engine::Keyboard::KeyWasPressed('N')) { m_adjustmentSpeedMultiplier *= 0.9f; }
	if (Engine::Keyboard::KeyWasPressed(' ')) {
		m_walkEnabled = !m_walkEnabled;
		m_camera.SetSpeed(m_camera.GetSpeed() * (m_walkEnabled ? 0.25f : 4.0f));
	}

	if (Engine::Keyboard::KeyWasPressed('U'))
	{ 
...
bool WorldEditor::UglyDemoCode()
{
	...
	m_xArrow.SetRotMat(Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_X));
	...
	m_yArrow.SetRotMat(Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_Y));
	...
	m_zArrow.SetRotMat(Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_Z));
...
void WorldEditor::AttachArrowsTo(Engine::GraphicalObject *pObj)
{
	...
	m_xArrow.SetRotMat(pObj->GetRotMat() * Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_X));
	m_xArrow.SetTransMat(Engine::Mat4::Translation(m_xArrow.GetRotMat() * X_ARROW_OFFSET + pObj->GetPos()));
	m_xArrow.CalcFullTransform();


	m_yArrow.SetRotMat(pObj->GetRotMat() * Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_Y));
	m_yArrow.SetTransMat(Engine::Mat4::Translation(m_yArrow.GetRotMat() * X_ARROW_OFFSET + pObj->GetPos()));
	m_yArrow.CalcFullTransform();


	m_zArrow.SetRotMat(pObj->GetRotMat() * Engine::Mat4::RotationToFace(BASE_ARROW_DIR, Engine::Vec3::PLUS_Z));
...
{
	if (pArrow == &m_xArrow) { return  Engine::Vec3::PLUS_X; }
	if (pArrow == &m_yArrow) { return  Engine::Vec3::PLUS_Y; }
	if (pArrow == &m_zArrow) { return  Engine::Vec3::PLUS_Z; }
