...
namespace Engine
{
	...
	{
		//if (!AreGridIndicesValid(gridX, gridY, gridZ)) { return nullptr; } // redundant check - valid called in get index, returns -1 if invalid
		int i = GetArrayIndexFromXYZIndices(gridX, gridY, gridZ);

		//GameLogger::Log(MessageType::ConsoleOnly, "i = %d\n", i);
		return i == -1 ? nullptr : &m_pData[m_pGridStartIndices[i]];
	}
	...
	{
		//if (!AreGridIndicesValid(gridX, gridY, gridZ)) { return -1; } // redundant check - valid called in get index, returns -1 if invalid
		int i = GetArrayIndexFromXYZIndices(gridX, gridY, gridZ);

		return  i == -1 ? -1 : m_pGridTriangleCounts[i];
	}
