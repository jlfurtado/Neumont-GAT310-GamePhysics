...
void EngineDemo::Update(float dt)
{
	...
	if (debug)
	{
		...
		if (toggleDrag)
		{
			...
			sprintf_s(&buffer2[0], 256, "K (spring const): %.2f\nR (resting length): %.2f\n", m_particleSpring.GetSpringConst(), m_particleSpring.GetRestLen());
		}
...
void EngineDemo::AlignObj(int index)
{
	...
	Engine::Vec3 pos = ORIGIN + (Engine::Vec3(-100.0f, 0.0f, 0.0f) * (float)index);
