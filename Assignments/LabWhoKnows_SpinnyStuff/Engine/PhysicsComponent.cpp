...
namespace Engine
{
	...
	bool PhysicsComponent::Update(float dt)
	{
		...
		if (m_particle.IsRotating())
		{
			m_pGobComp->GetGraphicalObject()->SetRotMat(Mat4::RotationAroundAxis(m_particle.GetRotationAxis(), m_particle.GetRadiansRotated()));
		}
	...
	void PhysicsComponent::StartSpin(float rotationsPerSecond, const Vec3 & axis)
	{
		m_particle.Rotate(rotationsPerSecond, axis);
	}
